...
Step 14: Google Chat as an Additional Interface

Goal
Add Google Chat as a second communication interface (in addition to SMS/WhatsApp) while reusing the existing ShopFront agent, state, and update pipeline.

Phase 1: Foundation (Channel Abstraction)

1. Unify channel model
Prompt:
```text
Update the messaging domain model to support Google Chat as an additional channel.
- In src/models/types.ts, extend Channel to include "google_chat".
- Extend InboundMessage with:
  - externalUserId?: string
  - externalSpaceId?: string
  - rawPayload?: unknown
- Ensure existing SMS/WhatsApp code still type-checks with strict TypeScript.
...
Prompt:
```text
Add a Prisma model to map channel identities to shops.
Create model ChannelIdentity with fields:
- id String @id @default(uuid())
- shopId String
- shop Shop @relation(fields: [shopId], references: [id])
- channel String
...
Add useful indexes/uniqueness for lookup:
- @@index([channel, phone])
- @@index([channel, externalUserId])
- @@unique([channel, phone])
- @@unique([channel, externalUserId])

Generate and apply migration, regenerate Prisma client, and add minimal helper queries in a new service src/services/channelIdentity.ts.
```

3. Messaging adapter interface
Prompt:
```text
Refactor src/services/messaging.ts to an adapter pattern while preserving current behavior.
Requirements:
- Keep exported sendMessage(msg) API unchanged.
- Route by msg.channel:
  - "sms" and "whatsapp" -> existing Twilio flow
  - "google_chat" -> placeholder adapter for now (throw NotImplemented with clear message)
- Add structured logs with channel and recipient.
- Ensure tests/build pass.
```

Phase 2: Google Chat App Plumbing

1. Inbound route
Prompt:
```text
Implement Google Chat inbound webhook route.
- Add POST /api/webhook/google-chat in src/routes/webhook.ts (or a dedicated route file).
- Parse Google Chat event payload into unified InboundMessage:
  - id from event/message id
  - body from text/message argument
  - channel = "google_chat"
  - externalUserId from sender user resource
  - externalSpaceId from space resource
- Return 200 JSON response expected by Google Chat.
- Skip advanced auth only behind env flag SKIP_GOOGLE_CHAT_VALIDATION=true for local testing.
- Add TODO comments where production request verification must be enforced.
```

2. Google request validation
Prompt:
```text
Add production-grade Google Chat request validation.
- Create src/lib/googleChatAuth.ts.
- Verify bearer token/JWT according to Google Chat app request auth guidance.
- Add env config keys needed for validation.
- In webhook route, reject invalid requests with 403.
- Add unit tests for valid/invalid auth paths.
```

3. Outbound sender
Prompt:
```text
Implement outbound Google Chat sender.
- Create src/services/googleChatMessaging.ts.
- Implement sendGoogleChatMessage({ spaceId, text }) using Google Chat REST API (service account auth).
- Integrate into sendMessage for channel "google_chat".
- Add required env vars in .env.example.
- Add robust error handling and structured logs.
```

Phase 3: Agent + State Integration

1. Conversation state keying
Prompt:
```text
Update conversation state/history utilities to support non-phone identities.
- Keep existing phone-based behavior unchanged.
- Add helper to build a canonical identity key:
  - sms/whatsapp: phone number
  - google_chat: externalUserId
- Update processMessage flow to use identity key for state/history/rate-limit.
- Ensure no regression to existing SMS tests.
```

2. Shop lookup + mapping
Prompt:
```text
Integrate ChannelIdentity lookup into processMessage.
Flow:
- For sms/whatsapp: existing phone lookup remains.
- For google_chat:
  - find ChannelIdentity by (channel, externalUserId)
  - if found, load linked shop
  - if not found, start onboarding mode
- On onboarding completion for google_chat, create ChannelIdentity link.
- Add integration tests for mapped and unmapped chat users.
```

3. Confirmation parity
Prompt:
```text
Ensure existing confirmation logic works for google_chat channel.
- Reuse yes/no text confirm/cancel behavior.
- Verify pendingAction flow works identically across sms/whatsapp/google_chat.
- Add tests for confirm, cancel, and new-message redirect on google_chat.
```

Phase 4: UX Behavior in Chat

1. MVP text responses
Prompt:
```text
Add Google Chat response formatting for MVP.
- Keep plain text responses only.
- Ensure outbound messages target the correct space/thread.
- Preserve existing response text from agent/router.
- Add logs for message_sent with channel=google_chat.
```

2. Help intent channel copy
Prompt:
```text
Add channel-aware help responses.
- For google_chat, help text should mention “Type updates directly in this chat”.
- For sms/whatsapp, keep current wording.
- Keep implementation centralized in router/help handler.
```

Phase 5: Testing & Validation

1. Parser + route tests
Prompt:
```text
Create tests for Google Chat webhook parsing and routing.
- Add tests file: tests/google-chat.webhook.test.ts
- Cases:
  - valid payload -> normalized InboundMessage
  - missing text -> graceful fallback
  - invalid auth -> 403 (when validation enabled)
...
Prompt:
```text
Add google_chat integration tests for end-to-end agent flow.
Test cases:
1) new google_chat user triggers onboarding
2) onboarding completion creates Shop + ChannelIdentity
3) existing mapped user updates service and confirms
4) website rebuild is triggered after mutation
Use mocks/fixtures where external APIs are required.
```

3. E2E simulation script
Prompt:
```text
Add scripts/test-google-chat-webhook.ts to simulate Google Chat events locally.
- Post fixture payloads to /api/webhook/google-chat
- Print status and response
- Support env toggle for auth validation skip
...
Prompt:
```text
Add feature flag ENABLE_GOOGLE_CHAT.
- If false, /api/webhook/google-chat returns 404 or 503 with clear message.
- If true, full route active.
- Add to .env.example and config validation.
```

2. Observability
Prompt:
```text
Extend logs/monitoring for Google Chat channel.
- Ensure key events include channel, externalUserId, spaceId when available:
  - message_received
  - intent_classified
  - message_sent
  - error
- Add Sentry tags for channel=google_chat and identity context.
```

3. Rollout checklist doc
Prompt:
```text
Create docs/GOOGLE_CHAT_ROLLOUT.md with a concrete launch checklist:
- GCP/Chat app setup
- required env vars
- local verification steps
- production smoke tests
- rollback plan (disable ENABLE_GOOGLE_CHAT)
